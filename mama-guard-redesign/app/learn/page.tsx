...
import { Search, Bookmark, Clock, ChevronRight, Play, Filter, X } from "lucide-react";

interface Article { id: string; title: string; category: string; readTime: number; type: "article" | "video"; weekRelevance?: [number, number]; bookmarked: boolean; excerpt: string; }
...
export default function LearnPage() {
  ...
  const getUserWeek = () => { try { const stored = localStorage.getItem("mamaguard_onboarding"); if (stored) { const data = JSON.parse(stored); return getGestationalWeek(data.dueDate); } } catch { /* ignore */ } return 0; };
      ...
      <main className="pt-20 pb-28 px-5">
        ...
        <div className="grid grid-cols-1 gap-4">
          {sortedArticles.map((article, index) => {
            ...
            return (
              <motion.button key={article.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} onClick={() => setSelectedArticle(article)} className="text-left bg-[var(--surface-primary)] rounded-2xl p-4 shadow-md border border-[var(--warm-200)]/60 active:scale-[0.98] transition-transform">
                <div className="flex items-start gap-3">
                  <div className={`w-16 h-16 rounded-xl flex items-center justify-center flex-shrink-0 ${article.type === "video" ? "bg-gradient-to-br from-[var(--rose-100)] to-[var(--rose-200)]" : "bg-gradient-to-br from-[var(--sage-100)] to-[var(--sage-200)]"}`}>{article.type === "video" ? <Play size={24} className="text-[var(--rose-600)]" /> : <Bookmark size={24} className="text-[var(--sage-600)]" />}</div>
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="absolute bottom-0 left-0 right-0 bg-[var(--bg-primary)] rounded-t-3xl max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-5">
                ...
                <div className="prose prose-sm max-w-none text-[var(--text-secondary)]">
                  ...
                  <p className="mb-4">The content would be personalized based on the user&apos;s current gestational week and health history, providing relevant and timely information.</p>
                  <div className="bg-[var(--amber-50)] border border-[var(--amber-200)] rounded-xl p-4 my-4">
                    <p className="text-xs font-semibold text-[var(--amber-700)] mb-1">Medical Disclaimer</p>
                    <p className="text-xs text-[var(--amber-800)]">This information is for educational purposes only and does not replace professional medical advice. Always consult your healthcare provider.</p>
                  </div>
                </div>
