  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        setUser(parsed);
        if (!parsed.name) router.replace("/onboarding");
      } catch { router.replace("/onboarding"); }
    } else router.replace("/onboarding");
    const checkins = localStorage.getItem("mamaguard_checkins");
    if (checkins) {
      try { const records: CheckInRecord[] = JSON.parse(checkins); if (records.length > 0) setLastCheckIn(records[records.length - 1]); } catch { /* ignore */ }
    }
  }, [router]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-[var(--bg-primary)] via-[var(--bg-secondary)] to-[var(--bg-cream)]">
      ...
      <main className="pt-20 pb-28 px-5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }} className="rounded-2xl bg-[var(--surface-primary)] p-5 shadow-md border border-[var(--warm-200)]/60">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-2xl font-bold text-[var(--text-primary)]">{lastCheckIn ? "1" : "0"}</div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }} className="mt-6 flex items-center justify-center gap-2 py-4">
          ...
          <span className="text-[11px] text-[var(--text-muted)]">Your data is encrypted and never shared without consent</span>
        </motion.div>
      </main>
